...
$headings = [
  'home' => [
    'text' => 'Speakers de primer nivel que marcaron tendencia en ediciones anteriores:',
    'class' => 'emms__fade-in speakers__title--lg',
  ],
  'registerHome' => [
    'text' => 'Speakers de primer nivel que marcaron tendencia en ediciones anteriores:',
    'class' => 'emms__fade-in speakers__title--md',
  ],
  'digital-trends' => [
    'text' => 'Speakers que brillaron en el EMMS',
    'class' => 'emms__fade-in speakers__title--md',
  ],
];

$currentHeading = $headings[$block['block']] ?? $headings['home'];
...
function getCTA($block, $isTransition)
{
  if ($isTransition) {
    switch ($block) {
      case 'home':
        return ['url' => '/digital-trends', 'label' => 'ASEGURA TU LUGAR'];
      case 'registerHome':
        return ['url' => '/digital-trends-registrado', 'label' => 'SÚMATE GRATIS'];
      case 'digital-trends':
        return ['url' => '#registro', 'label' => 'SÚMATE GRATIS'];
    }
  } else {
    switch ($block) {
      case 'home':
        return ['url' => '/digital-trends', 'label' => 'ÚNETE AL VIVO'];
      case 'registerHome':
        return ['url' => '/digital-trends-registrado', 'label' => 'ÚNETE AL VIVO'];
      case 'digital-trends':
        return ['url' => '#registro', 'label' => 'ÚNETE AL VIVO'];
    }
  }
}
$cta = getCTA($block['block'], $isTransition);
...
<section class="speakers emms__bg-section-10">
  <div class="emms__container--lg">
    ...
    <div class="spealers__header">
      <h2 class="speakers__title <?= $currentHeading['class'] ?>"><?= $currentHeading['text'] ?></h2>
    </div>
    ...
    if ($cta) {
      echo '<a href="' . $cta['url'] . '" class="emms__cta emms__cta--md emms__fade-in">' . $cta['label'] . '</a>';
    }
    ?>
